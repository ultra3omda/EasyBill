...
import { Plus, Search, CreditCard, Trash2, MoreVertical, Banknote, Building2, Wallet } from 'lucide-react';
...
const Payments = () => {
  ...
  const [formData, setFormData] = useState({
    customer_id: '', date: new Date().toISOString().split('T')[0], amount: 0,
    payment_method: 'cash', reference: '', allocations: [], notes: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...formData,
        type: 'received',
        allocations: formData.allocations.filter(a => a.selected && a.amount > 0).map(a => ({ invoice_id: a.invoice_id, amount: a.amount }))
      };
      
      await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/payments/?company_id=${currentCompany.id}`, {
        method: 'POST',
  ...
  const openCreate = () => {
    setFormData({ customer_id: '', date: new Date().toISOString().split('T')[0], amount: 0, payment_method: 'cash', reference: '', allocations: [], notes: '' });
    setPendingInvoices([]);
    setModalOpen(true);
  };

  const getMethodIcon = (method) => {
    const icons = { cash: <Banknote className="w-4 h-4" />, check: <CreditCard className="w-4 h-4" />, transfer: <Building2 className="w-4 h-4" />, card: <CreditCard className="w-4 h-4" />, e_dinar: <Wallet className="w-4 h-4" /> };
    return icons[method] || <Banknote className="w-4 h-4" />;
  };

  const getMethodLabel = (method) => {
    const labels = { cash: 'Espèces', check: 'Chèque', transfer: 'Virement', card: 'Carte bancaire', e_dinar: 'E-Dinar' };
    return labels[method] || method;
  };

  const filteredPayments = payments.filter(p => p.customer_name?.toLowerCase().includes(searchTerm.toLowerCase()) || p.number?.toLowerCase().includes(searchTerm.toLowerCase()));
  const stats = {
    total: filteredPayments.reduce((s, p) => s + (p.amount || 0), 0),
    cash: filteredPayments.filter(p => p.payment_method === 'cash').reduce((s, p) => s + (p.amount || 0), 0),
    ...
    <AppLayout>
      <div className="space-y-6" data-testid="payments-page">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Paiements reçus</h1>
            <p className="text-gray-500 mt-1">{filteredPayments.length} paiements</p>
          </div>
          <Button className="bg-violet-600 hover:bg-violet-700 text-white" onClick={openCreate} data-testid="create-payment-btn">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-sm text-gray-600">Total reçu</p><p className="text-2xl font-bold text-green-600">{stats.total.toFixed(2)} TND</p></div>
            </div>
          </Card>
          <Card className="p-6">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-violet-100 rounded-lg"><Banknote className="w-5 h-5 text-violet-600" /></div>
              <div><p className="text-sm text-gray-600">Espèces</p><p className="text-2xl font-bold text-violet-600">{stats.cash.toFixed(2)} TND</p></div>
            </div>
          </Card>
          <Card className="p-6">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-blue-100 rounded-lg"><CreditCard className="w-5 h-5 text-blue-600" /></div>
              <div><p className="text-sm text-gray-600">Chèques</p><p className="text-2xl font-bold text-blue-600">{stats.check.toFixed(2)} TND</p></div>
            </div>
          </Card>
          <Card className="p-6">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-orange-100 rounded-lg"><Building2 className="w-5 h-5 text-orange-600" /></div>
              <div><p className="text-sm text-gray-600">Virements</p><p className="text-2xl font-bold text-orange-600">{stats.transfer.toFixed(2)} TND</p></div>
            </div>
          </Card>
        </div>
        ...
        <Card>
          {loading ? (
            ...
            <div className="p-8 text-center">
              ...
              <Button onClick={openCreate} className="mt-4 bg-violet-600 hover:bg-violet-700"><Plus className="w-4 h-4 mr-2" /> Enregistrer</Button>
            </div>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {filteredPayments.map((payment) => (
                  <TableRow key={payment.id}>
                    <TableCell><div className="flex items-center gap-3"><div className="w-10 h-10 bg-green-100 rounded-lg flex items-center justify-center"><CreditCard className="w-5 h-5 text-green-600" /></div><span className="font-medium">{payment.number}</span></div></TableCell>
                    <TableCell>{payment.customer_name}</TableCell>
                    <TableCell>{payment.date ? new Date(payment.date).toLocaleDateString('fr-FR') : '-'}</TableCell>
                    <TableCell><Badge className="bg-gray-100 text-gray-800 flex items-center gap-1 w-fit">{getMethodIcon(payment.payment_method)} {getMethodLabel(payment.payment_method)}</Badge></TableCell>
                    <TableCell>{payment.reference || '-'}</TableCell>
                    <TableCell className="font-semibold text-green-600">+{(payment.amount || 0).toFixed(2)} TND</TableCell>
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem className="text-red-600" onClick={() => handleDelete(payment.id)}><Trash2 className="w-4 h-4 mr-2" /> Supprimer</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
                  </TableRow>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select value={formData.customer_id} onValueChange={(v) => setFormData({...formData, customer_id: v})}>
                  <SelectTrigger data-testid="payment-customer-select"><SelectValue placeholder="Sélectionner un client" /></SelectTrigger>
              ...
              <div>
                ...
                <Select value={formData.payment_method} onValueChange={(v) => setFormData({...formData, payment_method: v})}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="e_dinar">E-Dinar</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div><Label>Référence</Label><Input value={formData.reference} onChange={(e) => setFormData({...formData, reference: e.target.value})} placeholder="N° chèque, réf. virement..." /></div>
            </div>
                    ...
                    {pendingInvoices.map((inv, index) => (
                      <div key={inv.id} className="p-3 flex items-center gap-4 hover:bg-gray-50">
                        ...
                        <div className="text-right">
                          <p className="text-sm text-gray-500">Total: {inv.total.toFixed(2)} TND</p>
                          <p className="font-medium text-orange-600">Reste: {inv.balance_due.toFixed(2)} TND</p>
                        </div>
                        <div className="w-32">
                          <Input type="number" step="0.01" value={formData.allocations[index]?.amount || 0} onChange={(e) => handleAllocationChange(index, parseFloat(e.target.value) || 0)} placeholder="Montant" disabled={!formData.allocations[index]?.selected} />
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="flex justify-between items-center bg-violet-50 p-4 rounded-lg">
              ...
              <p className="text-3xl font-bold text-violet-600">{formData.amount.toFixed(2)} TND</p>
            </div>
            ...
            <DialogFooter>
              ...
              <Button type="submit" className="bg-violet-600 hover:bg-violet-700" data-testid="payment-submit">Enregistrer le paiement</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
