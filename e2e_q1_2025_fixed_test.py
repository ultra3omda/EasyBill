    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                if response_get.status_code == 200:
                    full_invoice = response_get.json()
                    full_invoice["status"] = "sent"
                    ...
                    if response.status_code == 200:
                        log_success(f"Facture {invoice1_data['number']} → statut 'sent' (sync comptable déclenchée)")
                    else:
                        log_warning(f"Erreur changement statut: {response.status_code}")
                else:
                    log_warning(f"Erreur récupération facture: {response_get.status_code}")
            else:
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                if response.status_code == 200:
                    log_success(f"Facture {invoice3_data['number']} → statut 'sent' (sync comptable déclenchée)")
            else:
                log_error(f"Erreur création facture: {response.status_code}")
        except Exception as e:
            log_error(f"Exception création facture: {str(e)}")
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                if response.status_code == 200:
                    log_success(f"✨ Facture fournisseur {supplier_invoice2_data['number']} → statut 'validated' (sync comptable déclenchée)")
                else:
                    log_warning(f"Erreur changement statut: {response.status_code}")
            else:
